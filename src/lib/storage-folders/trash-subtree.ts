/**
 * Soft-delete a storage_folders subtree: all descendant folders + files (backup_files) under a root.
 */
...
const TRASH_INPUT_CHUNK = 200;
const MAX_EXPANDED_FILES = 12_000;
const FOLDER_UPDATE_BATCH = 450;

function driveWideBackupFileQuery(
  db: Firestore,
...
export async function trashStorageFolderSubtree(
  db: Firestore,
  uid: string,
  folderId: string,
  opts?: { expectedVersion?: number }
): Promise<{ filesTrashed: number; foldersTrashed: number }> {
  ...
  if (root.lifecycle_state !== BACKUP_LIFECYCLE_ACTIVE) {
  ...
  const filesSnap = await driveWideBackupFileQuery(db, linkedDriveId, driveData, uid).get();
  ...
  if (uniqueFiles.length > MAX_EXPANDED_FILES) {
    throw new StorageFolderAccessError(
      `Too many files (${uniqueFiles.length}) in this folder tree. Delete in smaller steps or contact support.`,
      400
    );
  }
  ...
  if (uniqueFiles.length > 0) {
    for (let i = 0; i < uniqueFiles.length; i += TRASH_INPUT_CHUNK) {
      const chunk = uniqueFiles.slice(i, i + TRASH_INPUT_CHUNK);
      const result = await moveBackupFilesToTrashFromWebInput(db, uid, chunk, {
        source: "web",
        maxInputIds: TRASH_INPUT_CHUNK,
        maxExpandedIds: MAX_EXPANDED_FILES,
      });
      if (!result.ok) {
        throw new StorageFolderAccessError(result.err.error, result.err.status);
      }
      filesTrashed += result.expandedFileCount;
    }
  }
  ...
  for (const id of folderIdList) {
    ...
    if (data.lifecycle_state !== BACKUP_LIFECYCLE_ACTIVE) continue;
    const nextVersion =
  ...
  return { filesTrashed, foldersTrashed };
}
