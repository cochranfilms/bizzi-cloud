/**
 * POST — Materialize a photo favorites proofing list into Gallery Media (photographer only).
 */
...
export async function POST(
  ...
  if (g.gallery_type === "video") {
    return NextResponse.json(
      { error: "Use POST /api/galleries/{id}/selects/{listId}/materialize for video galleries" },
      { status: 400 }
    );
  }
  ...
  if (lt === "video_selects") {
    return NextResponse.json({ error: "List is a video selects list" }, { status: 400 });
  }
