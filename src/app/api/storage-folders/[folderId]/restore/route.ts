...
/** POST /api/storage-folders/[folderId]/restore — restore trashed subtree (files + folder rows). */
export async function POST(request: Request, ctx: RouteParams) {
  ...
  try {
    const summary = await restoreStorageFolderSubtree(db, auth.uid, folderId, {
      expectedVersion,
    });
